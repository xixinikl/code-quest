...
验收：`npm run test -- src/App.test.tsx --run -t "进入主线后进入教学地图"` 通过；`npm run verify:quick` 通过 11 个测试文件 / 195 个测试。浏览器录入式复核仍建议继续补完：从第二地点封存复述进入第三地点，再检查桌面与 390px 下 `下一地点预告` 是否无横向溢出、无白色断层。
