...
- [x] 修复路线大厅虽然有当前附近星图和任务板，但长期成长仍容易被看成“课程列表”的问题。
- [x] 在世界地图下新增 `成长星约`，默认展示当前路线接下来三次会解锁的伙伴、宠物或装备。
- [x] 每个解锁条目显示章节编号、收集状态、解锁对象、说明和 `能力印记`，让用户知道通关不是只拿 XP，而是收集角色和可面试复盘的能力。
- [x] 成长星约跟随 `AI 应用开发 / Java 后端 / 前端工程` 路线和当前进度自动变化，不写死单一路线。
- [x] 手机端改为单列，避免三张收集预告挤在一行造成阅读负担。
- [x] 补充回归测试：AI 主线进入路线大厅后必须显示 `成长星约`、`未来三次解锁`、`伙伴 · 档案馆记录员`、`宠物 · 灵感萤火`、`伙伴 · 回廊守卫` 和能力印记。
- [x] 真实浏览器复核：隔离 API `4425`、Vite `5297`，从新用户序章进入任务简报，真实页面显示 `成长星约`、`未来三次解锁`、三次解锁预告和能力印记；桌面和 390px 手机宽度均无横向溢出；截图：`output/playwright/r372-route-growth-contract-desktop.png`、`output/playwright/r372-route-growth-contract-mobile.png`。
