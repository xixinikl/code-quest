...
### R371：第 1 章线索点击加入获得反馈

- [x] 修复用户点击探索线索后，虽然能看到线索文字和伙伴回应，但缺少“我刚刚获得了什么”的即时游戏反馈。
- [x] 在线索结果中新增 `线索获得` 回执，显示 `证据 +1`、收进卷宗的能力说明、伙伴默契进度和下一步追证据方向。
- [x] 第 1 章点击 `检查绿色灯牌` 后，会显示 `档案馆记录员 1/2`、`收进卷宗`、`下一步追证据` 和 `后端接口 → 数据层函数`。
- [x] 保持暗色 RPG 风格，桌面三列、手机单列，避免把线索反馈变成普通后台提示。
- [x] 补充回归测试：第 1 章点击首个线索后必须显示 `线索获得`、`证据 +1`、`检查绿色灯牌`、`档案馆记录员 1/2` 和下一步证据方向。
- [x] 真实浏览器复核：隔离 API `4424`、Vite `5296`，从新用户序章进入第 1 章，点击 `检查绿色灯牌` 后真实页面显示 `线索获得`、`证据 +1 · 检查绿色灯牌`、`档案馆记录员 1/2` 和 `后端接口 → 数据层函数`；桌面和 390px 手机宽度均无横向溢出；截图：`output/playwright/r371-clue-reward-feedback-desktop.png`、`output/playwright/r371-clue-reward-feedback-mobile.png`。

验收：`npm run test -- src/App.test.tsx --run -t "进入主线后进入教学地图"` 通过；`npm run verify:quick` 通过 11 个测试文件 / 203 个测试。

### R370：第 1 章剧情探索加入本章收集契约

- [x] 修复用户在探索页只能看到 XP/HUD，缺少“这一章会收集什么、通关会解锁谁”的养成动机问题。
- [x] 在剧情舞台镜头后新增 `本章收集契约`，展示通关收藏、线索印记和能力印记。
- [x] 通关收藏复用现有 `teachingCompanions` 数据，不另起奖励系统；第 1 章显示 `伙伴 · 档案馆记录员`，后续章节可自动显示各自伙伴/宠物/装备。
- [x] 线索印记显示总进度和当前地点进度；能力印记使用当前证据出口，强调结案复盘和面试素材不靠字数或感觉冒充掌握。
- [x] 补充回归测试：第 1 章进入主线后必须显示 `本章收集契约`、`通关不是只拿 XP`、`伙伴 · 档案馆记录员`、`线索印记`、`0/8`、`能力印记` 和当前证据出口。
- [x] 真实浏览器复核：隔离 API `4423`、Vite `5295`，从新用户序章进入第 1 章，真实页面存在 `本章收集契约`，文案包含 `伙伴 · 档案馆记录员`、`线索印记 0/8` 和能力印记；桌面和 390px 手机宽度均无横向溢出；控制台 0 error / 0 warning；截图：`output/playwright/r370-collection-contract-desktop.png`、`output/playwright/r370-collection-contract-mobile.png`。

验收：`npm run test -- src/App.test.tsx --run -t "进入主线后进入教学地图"` 通过；`npm run verify:quick` 通过 11 个测试文件 / 203 个测试。

### R369：第 1 章剧情探索加入人物舞台镜头

- [x] 修复第 1 章探索页虽然已有背景、角色和导航，但关键路径仍更像信息卡堆叠的问题：在地点航线后新增 `剧情舞台镜头`。
- [x] 舞台镜头随当前 `scene.image / scene.speaker / scene.mentor` 变化，展示当前地点、当前登场角色、剧情对白、这一幕先懂什么、下一幕交接到哪里。
- [x] 人物图使用现有同风格角色资产，不新造风格断层；桌面左侧剧情与学习锚点、右侧角色立绘，手机收成单列头像式对话。
- [x] 保留原有 `h1` 作为唯一章节主标题，舞台标题只做视觉标题，避免无障碍 heading 重复。
- [x] 补充回归测试：第 1 章进入主线后必须显示 `剧情舞台镜头`、`第 1/4 幕`、`舞台记录员`、`先懂这一句`、`下一幕交接` 和 `传送门大厅`。
- [x] 真实浏览器复核：隔离 API `4422`、Vite `5294`，从新用户序章进入第 1 章，真实页面存在 `剧情舞台镜头 / 当前登场角色 / 这一幕学习锚点`；桌面和 390px 手机宽度均无横向溢出；控制台 0 error / 0 warning；截图：`output/playwright/r369-scene-spotlight-desktop.png`、`output/playwright/r369-scene-spotlight-mobile.png`。

验收：`npm run test -- src/App.test.tsx --run -t "进入主线后进入教学地图"` 通过；`npm run verify:quick` 通过 11 个测试文件 / 203 个测试。

### R368：第 1 章剧情和教学页加入本章导航针

- [x] 修复用户进入第 1 章后，虽然有地点航线和流程卷轴，但仍可能不知道“刚才讲到哪、现在要干什么、下一步交给谁”的问题。
- [x] 在剧情探索页和教学桥页都新增 `本章导航针`，固定显示 `刚从哪里来 / 现在做什么 / 完成后交给谁`。
- [x] 导航针会显示当前站编号、路线标题和“当前只要交出”的证据出口，让用户先抓住本幕目标，再探索线索或读代码。
- [x] 桌面三列、手机单列，保持暗色 RPG 风格，不增加白色后台断层。
- [x] 补充回归测试：第 1 章进入主线后必须显示 `本章导航针`、`剧情调查`、`现在做什么`、`完成后交给谁` 和 `当前只要交出`。
- [x] 真实浏览器复核：隔离 API `4421`、Vite `5293`，从新用户序章进入第 1 章，确认学习页显示 `本章导航针`、`刚从哪里来`、`现在做什么`、`完成后交给谁`；390px 手机宽度 `scrollWidth = 390`，无横向溢出；控制台 0 error / 0 warning；截图：`output/playwright/r368-chapter-navigation-needle-mobile.png`。

验收：`npm run test -- src/App.test.tsx --run -t "进入主线后进入教学地图"` 通过；`npm run verify:quick` 通过 11 个测试文件 / 203 个测试。

### R367：简报页世界地图默认只显示当前附近星图

- [x] 修复用户进入任务简报后，右侧一次看到 15 个章节按钮、任务板、卷宗和折叠区而容易发晕的问题。
- [x] `AI 应用开发 / Java 后端 / 前端工程` 可进入路线在未通关时，默认世界地图只展示当前章节附近最多 5 个地点；完整路线仍保留在 `后续主线关卡` 折叠星图里。
- [x] 主线全部通关后仍显示完整已点亮世界地图，保证结业授勋页能看到 15/15 的完成反馈。
- [x] 简报文案从“点击章节查看卷宗”改为“先看当前附近星图，完整路线收在下方”，让新手知道第一屏只需要关注当前委托。
- [x] 补充回归测试：AI 主线默认世界地图必须显示 `数据断层` 到 `一致性熔炉` 这段附近地点，但不能在默认地图里显示终章 `面试议会`。

验收：`npm run test -- src/App.test.tsx --run -t "进入主线后进入教学地图"` 通过。

### R366：Lab 首屏加入本关面试复盘稿

- [x] 修复用户拿到证据任务后，仍可能不知道“这关怎么讲给面试官听”的问题：在 `本关证据任务` 后新增 `本关面试复盘稿`。
- [x] 面试复盘稿从路线章节事实源自动生成 `情境 / 行动 / 结果 / 表达` 四段，不写死第 5 章，后续 AI、Java、前端路线都可复用。
- [x] 第 5 章真实 Lab 会显示：情境是用户连点、网络重试、多人同时编辑；行动是用重复请求、数据库记录数量和测试结果证明；结果是连续点击、刷新重试和并发请求不生成重复核心数据；表达是只靠前端禁用不够，还需要后端约束。
- [x] 增加 `追问准备`：如果面试官问“怎么证明不是猜的”，提示拿出本关记录的证据材料。
- [x] 样式保持暗色 RPG，桌面四格，手机单列；390px 浏览器验证无横向溢出。
- [x] 补充回归测试：第 5 章 Lab 必须显示面试复盘稿四段、工作情境、证据行动、面试表达和追问准备。
- [x] 真实浏览器复核：默认 Vite `5284` 打开 `#chapter-5` 后可见 `本关面试复盘稿`；390px 手机宽度 `scrollWidth = 390`；控制台 0 error / 0 warning；截图：`output/playwright/r366-chapter5-interview-replay.png`、`output/playwright/r366-chapter5-interview-replay-mobile.png`。

验收：`npm run test -- src/App.test.tsx --run -t "第 5 章 Lab 首屏会给出数据一致性|第 3 章 Lab 首屏会讲清登录态|第 4 章 Lab 首屏会解释接口错误名词"` 通过；`npm run verify:quick` 通过 11 个测试文件 / 203 个测试。
...
- [x] 导师卡随当前代码行动态生成三段：`这行代码能帮你`、`这行代码还不能`、`下一步追证据`。
- [x] 如果当前行涉及 `fetch / response.ok`，会提示下一步看 Network 的 method、payload、status 和 response body；如果涉及数据库，会提示看数据库查询和测试报告；如果涉及日志/错误，会提示看后端日志和失败路径测试。
- [x] 位置放在代码块和项目路径前，保证用户先理解“代码只是线索，不是结案书”，再去看文件路径和完整代码。
- [x] 样式保持暗色 RPG，桌面三列，手机单列；390px 浏览器验证无横向溢出。
- [x] 补充回归测试：代码导读页必须显示 `代码证据导师卡`、三段证据说明和“代码是线索，不是结案书”；第 1 章读到 `response.ok` 时必须提示继续看 Network/日志/数据库证据。
- [x] 真实浏览器复核：在 Vite 浏览器内挂载 `GuidedCodeTour` 组件，可见 `代码证据导师卡`；390px 手机宽度 `scrollWidth = 390`；控制台 0 error / 0 warning；截图：`output/playwright/r364-code-evidence-mentor.png`、`output/playwright/r364-code-evidence-mentor-mobile.png`。

验收：`npm run test -- src/App.test.tsx --run -t "代码导读会把关键行|第一章代码导读会使用|第一章代码导读完成后"` 通过；`npm run verify:quick` 通过 11 个测试文件 / 203 个测试。
...
- [x] 证据任务读取当前章节 `evidenceTask / acceptanceAction` 和 Lab 结案事实 `proved / recorded / pending`，不写死第 5 章。
- [x] 第 5 章真实 Lab 会显示：用 `重复请求、数据库记录数量和测试结果` 证明没有重复写入；验收动作是 `连续点击、刷新重试和并发请求` 都不会生成重复核心数据；结案证据是同一用户同一 `clientMutationId` 最终只产生一条核心草稿。
- [x] 证据任务放在流程路线、名词小抄之后，证据包之前，形成 `知道路线 → 看懂术语 → 明确证据 → 进入材料` 的首屏学习顺序。
- [x] 样式保持暗色 RPG，桌面三列，手机单列，不新增白色后台断层。
- [x] 补充回归测试：第 5 章 Lab 必须显示数据一致性的证据任务、验收动作和结案证据。
- [x] 真实浏览器复核：默认 API `4317`、Vite `5284`，打开 `#chapter-5` 并补齐本地教学记录后进入真实 Lab，可见 `本关证据任务`；控制台 0 error / 0 warning；截图：`output/playwright/r362-chapter5-evidence-quest.png`。

验收：`npm run test -- src/App.test.tsx --run -t "第 2 章 Lab 会说明工作|第 3 章 Lab 首屏会讲清登录态|第 4 章 Lab 首屏会解释接口错误名词|第 5 章 Lab 首屏会给出数据一致性|岗位 Lab 默认收束辅助资料"` 通过；`npm run verify:quick` 通过 11 个测试文件 / 203 个测试。

### R361：Lab 首屏加入本关名词小抄

- [x] 修复新手进入 Lab 后看到 `Cookie / 状态码 / 400 / 500 / 日志` 等名词仍可能发懵的问题：在 `本关流程路线` 后新增 `本关名词小抄`。
- [x] 名词小抄读取路线章节 `glossary`，并使用统一的新手解释表；不写死第 4 章，15 章常见术语都有解释兜底。
- [x] 第 4 章真实 Lab 会显示 `请求参数 / 状态码 / 400 / 500 / 日志 / 错误边界`，并解释请求参数是前端交给接口的数据、400 是参数问题、500 要查后端日志和异常栈。
- [x] 名词小抄放在流程路线之后、证据包之前，先帮用户理解词，再进入真实材料阅读。
- [x] 样式保持暗色 RPG，桌面自动网格，手机单列，不新增白色后台断层。
- [x] 补充回归测试：第 4 章 Lab 必须显示接口错误名词和新手解释，同时保留本关流程路线。
- [x] 真实浏览器复核：默认 API `4317`、Vite `5284`，打开 `#chapter-4` 并补齐本地教学记录后进入真实 Lab，可见 `本关名词小抄`；控制台 0 error / 0 warning；截图：`output/playwright/r361-chapter4-glossary.png`。

验收：`npm run test -- src/App.test.tsx --run -t "第 2 章 Lab 会说明工作|第 3 章 Lab 首屏会讲清登录态|第 4 章 Lab 首屏会解释接口错误名词|岗位 Lab 默认收束辅助资料"` 通过；`npm run verify:quick` 通过 11 个测试文件 / 202 个测试。

### R360：Lab 首屏加入本关流程路线

- [x] 修复用户进入 Lab 后仍可能不知道“谁把什么交给谁”的问题：在冒险日志后新增 `本关流程路线`。
- [x] 流程路线读取当前 `LabConfig.flowItems`，不写死第 2 或第 3 章；每章会显示自己的交接节点、当前交接和下一棒。
- [x] 第 3 章真实 Lab 会显示 `用户 → 登录接口 → 浏览器凭证 → 受保护接口 → 服务端会话 → 面试表达`，当前交接显示 `登录接口 把「返回 200」交给 浏览器凭证`。
- [x] 第 2 章真实 Lab 同步显示 `用户 / 写 Brief / 前端 / 提交方向 / 后端规划器 / 筛候选`，让 Project Brief 关卡也能先看完整产品链路。
- [x] 样式保持暗色 RPG，桌面自适应短节点，手机在区块内部横向滑动，不制造页面横向溢出。
- [x] 补充回归测试：第 2 章 Lab 必须显示本关流程路线；第 3 章 Lab 必须显示登录态路线、工作背景、Agent 委托和面试表达。
- [x] 真实浏览器复核：默认 API `4317`、Vite `5284`，打开 `#chapter-3` 并补齐本地教学记录后进入真实 Lab，可见 `本关流程路线`；控制台 0 error / 0 warning；截图：`output/playwright/r360-chapter3-route-compass.png`。

验收：`npm run test -- src/App.test.tsx --run -t "第 2 章 Lab 会说明工作|第 3 章 Lab 首屏会讲清登录态|岗位 Lab 默认收束辅助资料"` 通过；`npm run verify:quick` 通过 11 个测试文件 / 201 个测试。

### R359：Lab 首屏加入本关职业产出

- [x] 修复第 2 章实战入口虽然有证据链，但用户仍可能不知道“学这个工作里怎么用、怎么交给 Agent、面试怎么讲”的问题：在 `任务导演台` 后新增 `本关职业产出`。
- [x] 职业产出直接读取路线章节事实源 `workBackground / agentCollaboration / interviewReview`，不为第 2 章写死孤立文案；后续 AI、Java、前端路线进入 Lab 时会按各自章节显示。
- [x] 第 2 章真实 Lab 会显示 `工作里怎么用 / 怎么交给 Agent / 面试怎么讲`，对应 `真实工作里不会只说“做个 AI 功能”`、`让 Agent 先输出 Brief 和验收口径`、`我把模糊 AI 想法拆成可开发链路`。
- [x] 样式沿用暗色 RPG 金绿系统，桌面三列、手机单列，避免白色后台断层；区块放在任务导演台之后，不抢当前作答区域。
- [x] 补充回归测试：第 2 章 Lab 必须同时显示 `实战入场证据包`、`本关同行小队`、`场景运镜` 和 `本关职业产出`。
- [x] 真实浏览器复核：默认 API `4317`、Vite `5284`，打开 `#chapter-2` 并补齐本地教学记录后进入真实 Lab，可见 `本关职业产出`；控制台 0 error / 0 warning；截图：`output/playwright/r359-chapter2-career-outcome.png`。

验收：`npm run test -- src/App.test.tsx --run -t "第 2 章 Lab 会说明工作|第 1 章剧情调查完成后刷新深链会回到 Lab|岗位 Lab 默认收束辅助资料"` 通过；`npm run verify:quick` 通过 11 个测试文件 / 200 个测试。

### R358：Lab 首屏加入场景运镜

- [x] 修复真实 Lab 虽然会换背景和角色，但用户不一定知道“下一幕为什么换地点”的问题：在 `本关同行小队` 后新增 `场景运镜`。
- [x] 场景运镜显示 `当前镜头`、`下一地点`、`角色切换`，从当前 `LabStep.scene` 和下一步 scene 自动生成，不写死第 1 章。
- [x] 第 1 章真实 Lab 会显示当前 `前端讯号窗`、下一地点 `请求中转门`，以及角色从 `传送门书记官` 切到 `接口接待员`。
- [x] 桌面三列、手机单列，缩略背景取当前/下一地点图片，强化“场景在变、任务在推进”的剧情感。
- [x] 补充回归测试：第 1 章剧情调查完成后刷新深链回到 Lab 时，必须显示 `场景运镜`、当前地点、下一地点和角色切换。
- [x] 真实浏览器复核：默认 API `4317`、Vite `5284`，打开 `#chapter-1` 后可见 `场景运镜`；控制台 0 error / 0 warning；截图：`output/playwright/r358-lab-scene-travel.png`。

验收：`npm run test -- src/App.test.tsx --run -t "第 1 章剧情调查完成后刷新深链会回到 Lab|岗位 Lab 默认收束辅助资料"` 通过；`npm run verify:quick` 通过 11 个测试文件 / 199 个测试。

### R357：Lab 首屏加入同行小队与收集预告

- [x] 修复真实 Lab 首屏“有证据解释但游戏收集感还弱”的问题：在 `实战入场证据包` 后新增 `本关同行小队`。
- [x] 同行小队读取当前 `LabConfig` 对应路线章节，显示 `本幕同行`、`通关收藏`、`下一位会遇见` 三张卡，不写死第 1 章；AI、Java、前端路线都复用同一套章节/奖励数据。
- [x] 第 1 章真实 Lab 会显示 `档案馆记录员`、`伙伴 · 档案馆记录员` 和下一位 `宠物 · 灵感萤火`，让用户一进实战就知道通关会解锁角色/宠物与能力图鉴。
- [x] 手机端同行小队单列展示，桌面三列展示，保持暗色 RPG 风格；不新增白色后台断层。
- [x] 补充回归测试：第 1 章剧情调查完成后刷新深链回到 Lab 时，必须显示 `本关同行小队`、`通关收藏`、`下一位会遇见` 和 `宠物 · 灵感萤火`。
- [x] 真实浏览器复核：默认 API `4317`、Vite `5284`，打开 `#chapter-1` 后可见 `同行小队`；控制台 0 error / 0 warning；截图：`output/playwright/r357-lab-companion-squad.png`。

验收：`npm run test -- src/App.test.tsx --run -t "第 1 章剧情调查完成后刷新深链会回到 Lab|岗位 Lab 默认收束辅助资料"` 通过；`npm run verify:quick` 通过 11 个测试文件 / 199 个测试。

### R356：刷新直达 Lab 时显示实战入场证据包

- [x] 修复 R355 的体验断点：完成页有证据包，但章节刷新/深链恢复会直接进入 Lab，用户仍可能看不到“刚才学的证据要怎么带进实战”。
- [x] 在真实 `Lab` 首屏新增 `实战入场证据包`，放在冒险日志之后、任务导演台之前，显示 `从剧情带来 / 当前只看 / 能证明与不能证明 / 交给下一棒`。
- [x] 证据包读取当前 `LabConfig`、当前步骤、流程棒和材料导览，不写死第 1 章；第 1 章会显示 `任务委托 → 前端`、`fetch('/api/canvases'...)`、还不能证明数据库写入，以及下一棒后端接口要接待请求。
- [x] 区块只在 Lab 前两步显示，后续步骤不持续占屏；手机端两列收缩，保持暗色 RPG 样式，不新增白色后台断层。
- [x] 补充回归测试：第 1 章剧情调查完成后刷新深链回到 Lab 时，必须显示 `实战入场证据包` 和四张证据卡；岗位 Lab 默认收束测试保持通过。
- [x] 真实浏览器复核：默认 API `4317`、Vite `5284`，打开 `#chapter-1` 后可见 `实战入场证据包`，控制台 0 error / 0 warning；截图：`output/playwright/r356-lab-entry-evidence-pack.png`。

验收：`npm run test -- src/App.test.tsx --run -t "第 1 章剧情调查完成后刷新深链会回到 Lab|岗位 Lab 默认收束辅助资料"` 通过；`npm run verify:quick` 通过 11 个测试文件 / 199 个测试。

### R355：教学完成页加入带入实战的证据包

- [x] 修复教学完成后直接进入实战容易断片的问题：`TeachingBridge` 的完成页新增 `带入实战的证据包`，在按钮前先交代起点、关键代码、下一步验证和实战边界。
- [x] 证据包从真实场景数据生成：起点读取 `projectMap.nodes[0]`，代码证据读取当前关卡 `codeFocus`，下一步验证读取最后一个代码焦点或项目地图终点，避免写死第 1 章文案。
- [x] 第 1 章会明确提示 `frontend/SaveCanvasButton.jsx` 只是关键代码证据，下一步仍要用 Network、日志、数据库或测试结果闭环；实战边界明确 `只改沙盒，不碰真实项目`。
- [x] 补充回归测试：第 1 章教学进度全 completed 时，完成页必须显示 `带入实战的证据包`、起点别忘、代码只带关键行、下一步验证、实战边界和 Network/日志/数据库/测试结果。
- [x] 真实浏览器复核：默认 API `4317`、Vite `5284` 下，`#chapter-2` 可恢复到真实实战页，页面无业务控制台错误；当前深链恢复逻辑会把已记录剧情进度的章节直接送入 Lab，因此完成页证据包本轮以组件回归测试为直接证据，后续若要浏览器直达完成页，应先调整或专门隔离恢复策略。

验收：`npm run test -- src/App.test.tsx --run -t "带入实战的证据包|进入实战前会显示作战简报"` 通过；`npm run typecheck` 通过；`npm run verify:quick` 通过 11 个测试文件 / 199 个测试。浏览器截图：`output/playwright/r355-chapter2-lab.png`。

### R354：代码导读完成后补证据边界交接复盘

- [x] 修复代码导读完成后用户可能忘记“刚才看懂了什么、下一站为什么继续”的问题：`伙伴证据收录` 弹层在代码步骤完成后新增 `代码证据交接复盘`。
- [x] 复盘卡把当前代码步骤拆成 `刚才看懂 / 能证明 / 还不能证明 / 下一站带着它查`，明确第 1 章前端代码只能证明前端看到 `response.ok` 后亮成功提示，还不能证明真实环境稳定保存。
- [x] 下一站交接使用真实 `nextStep`，第 1 章会明确带着前端证据进入 `路由：只看 saveCanvas 调用与 return 201`，避免用户忘记为什么从前端走到后端。
- [x] 补充回归测试：第一章恢复到代码导读，点击 `我看懂了，继续下一步` 后，弹层必须显示代码证据交接复盘、能证明/还不能证明、下一站和 Network/后端日志/数据库记录边界。
- [x] 真实浏览器复核：默认 API `4317`、临时 SQLite `/tmp/code-quest-r354.sqlite`、Vite `5283` 下，`#chapter-1` 从 `前端：只看 fetch 与 response.ok` 点击完成后弹出 `代码证据交接复盘`。390×844 和 1280×900 均无横向溢出、无白色块；背景 `rgb(7, 12, 20)`，控制台 0 error、0 warning。

验收：`npm run test -- src/App.test.tsx --run -t "代码导读完成后|第一章代码导读会使用|代码导读会把关键行"` 通过；`npm run typecheck` 通过；`npm run verify:quick` 通过 11 个测试文件 / 198 个测试。

### R353：代码导读页新增“当前只读这一行”契约

- [x] 修复代码阅读页仍容易让新手被整段代码压住的问题：在关键代码块前新增 `当前只读这一行` 区域，先用人话说明当前行、收到什么、处理什么、交出什么，再让用户看代码。
- [x] 新区域直接复用当前活跃行的读码交接和证据锚点，显示 `收到 / 处理 / 交出 / 为什么看 / 下一证据`，并明确提示 `不要现在读全文件`，引导用户逐行推进。
- [x] 样式保持暗色神秘 RPG，不新增白色后台卡；390px 手机下 `当前只读这一行`、任务三栏和证据说明会单列展示，避免横向扫读。
- [x] 补充回归测试：第 2 章代码导读和第 1 章保存链路都必须显示 `当前只读这一行`、`不要现在读全文件`、收到/处理/交出和下一证据。
- [x] 真实浏览器复核：默认 API `4317`、临时 SQLite `/tmp/code-quest-r353.sqlite`、Vite `5282` 下，`#chapter-1` 恢复到 `前端：只看 fetch 与 response.ok` 代码导读页，显示新增区域。390×844 手机视口 `scrollWidth = clientWidth = 390`，桌面 1280 也无横向溢出；暗色背景 `rgb(7, 12, 20)`，无白色块，控制台 0 error、0 warning。

验收：`npm run test -- src/App.test.tsx --run -t "代码导读会把关键行|第一章代码导读"` 通过；`npm run typecheck` 通过；`npm run verify:quick` 通过 11 个测试文件 / 197 个测试。

### R352：跨电脑拉取交接更新到当前分支基线

- [x] 刷新根目录 `HANDOFF.md` 的另一台电脑拉取说明：顶部新增可直接复制给协作者的短说明，明确仓库、分支、不要用默认分支、三方 hash 核对和拉完验证命令。
- [x] 把旧的 `b42a904` 判断更新为当前功能基线 `865e721 feat(rpg): anchor player role on intro`，避免另一台电脑只拉到早期结案菜单提交就误以为完整。
- [x] 交接里明确“可以拉下来继续开发，但不建议直接合并 main”：另一台电脑应从 `cx/ai-career-rpg-home` 继续，或再切自己的 `cx/...` 分支，准备合并前必须跑完整验证和做 PR 审查。
- [x] 保留首次 clone、已有仓库更新、本地有改动时保护现场、拉完确认完整、启动开发和合并判断的照抄命令。

验收：`HANDOFF.md` 中 “另一台电脑怎么拉” 与顶部拉取卡均指向 `cx/ai-career-rpg-home` 和 `865e721` 之后的当前基线；后续推送后另一台电脑以 `git ls-remote origin refs/heads/cx/ai-career-rpg-home` 的 hash 为最终准绳。

### R351：新手首屏加入玩家身份契约

- [x] 修复新用户第一屏“像剧情标题，但还不够明确自己是谁”的问题：序章第一屏新增 `玩家身份契约`，打开首页就能看到 `见习 AI 调试师`、成长目标、第一关任务和首位伙伴。
- [x] 身份契约把项目目标讲成人话：从看不懂项目，练到能读证据、定位 bug、验收 Agent 交付；第一关先追踪一次保存请求，从前端灯牌走到数据库档案。
- [x] 首屏继续复用档案馆记录员立绘和暗色剧情舞台，不新增白色工具卡；手机端单列显示，避免信息挤在三列里。
- [x] 补充回归测试：进入主线 helper 必须先看到 `玩家身份契约`，并断言 `见习 AI 调试师`、成长目标、定位 bug、第一关任务和首位伙伴都在首屏。
- [x] 真实浏览器复核：隔离 API `4431`、临时 SQLite `/tmp/code-quest-r352.sqlite`、Vite `5281` 下，根路径首页显示 `玩家身份契约 / 见习 AI 调试师 / 成长目标 / 第一关任务 / 首位伙伴`。390×844 手机视口 `scrollWidth = clientWidth = 390`，暗色背景 `rgb(7, 12, 20)`，无白色大块；控制台 0 error、0 warning，仅 React DevTools info。

验收：`npm run test -- src/App.test.tsx --run -t "职业路线封面先以剧情方式显示|进入主线后进入教学地图"` 通过；`npm run typecheck` 通过；`npm run verify:quick` 通过 11 个测试文件 / 197 个测试。

### R350：成长档案加入下一幕预告转场

- [x] 修复通关后“只有文字菜单，不像剧情游戏转场”的问题：真实 `成长档案结案` 页在结案后冒险菜单之后新增 `下一幕预告`，用下一章的场景背景、主题、工作背景、流程交接和登场伙伴/宠物把剧情接上。
- [x] 下一幕预告复用路线事实源，不新增一套易漂移配置：章节主题来自 `CareerChapter.theme`，剧情来自 `storyScene`，工作背景来自 `workBackground`，流程来自 `flow`，登场角色/宠物来自 `companionUnlock`，场景图来自 `getChapterDossierBackground`。
- [x] 补强历史场景 id 兼容：`findRouteChapterTargetForScenarioId` 现在会同时识别描述性 id 和旧 `case-00x` id，避免旧记录或直接渲染路径找不到所属路线，导致下一幕预告不出现。
- [x] 新增暗色剧情海报样式：桌面横向展示“下一章剧情 + 登场角色”，手机单列展示，不新增白色后台断层。
- [x] 补充回归测试：成长档案页必须显示 `下一幕预告`，并包含下一关主题、下一位登场、角色名和下一关流程交接。
- [x] 真实浏览器复核：隔离 API `4430`、临时 SQLite `/tmp/code-quest-r351.sqlite`、Vite `5280` 下，从 `#chapter-1` 准备已通过 attempt，点击 `生成成长档案` 后真实结案页显示 `下一幕预告 · AI 应用开发 / 第 2 关 · AI 点子为什么空泛 / 创意工坊 / 下一位登场 / 宠物 · 灵感萤火`。390×844 手机视口 `scrollWidth = clientWidth = 390`，暗色背景 `rgb(7, 12, 20)`，无白色大块；控制台 0 error、0 warning，仅 React DevTools info。

验收：`npm run test -- src/App.test.tsx --run -t "实战通关后的成长档案|提交成功后停留"` 通过；`npm run typecheck` 通过；`npm run verify:quick` 通过 11 个测试文件 / 197 个测试。

### R349：成长档案加入结案后冒险菜单

- [x] 修复通关后仍可能迷路的问题：真实 `成长档案结案` 页在收藏解锁之后新增 `结案后冒险菜单`，明确告诉用户下一步可以去 `路线图`、`面试复盘册`、`迁移复测`，而不是只看到一个回路线图按钮。
- [x] 菜单文案把三条路讲成人话：`查看伙伴图鉴和下一章`、`把本关证据整理成回答`、`证明不是只会这一题`，继续保持“学习不是拿到 XP 就结束”的边界。
- [x] 新菜单使用暗色 RPG 卡片样式，保持与成长档案、收藏解锁和迁移口令一致；手机端单列显示，避免结案页出现白色断层或需要左右扫读。
- [x] 补充回归测试：真实提交后停留在成长档案页时，必须显示 `结案后冒险菜单` 三条下一步，并且按钮文案必须指向 `查看伙伴与下一章`。
- [x] 交接文档继续放在仓库根目录 `HANDOFF.md`，顶部写明另一台电脑首次 clone、已有仓库更新、本地有改动时保护现场、hash 核对、Node 版本、验证命令和当前不能直接合并的处理方式。
- [x] 真实浏览器复核：隔离 API `4429`、临时 SQLite `/tmp/code-quest-r350.sqlite`、Vite `5279` 下，从 `#chapter-1` 准备已通过 attempt，点击 `生成成长档案` 后真实结案页显示 `结案后冒险菜单 / 查看伙伴图鉴和下一章 / 把本关证据整理成回答 / 证明不是只会这一题`。390×844 手机视口 `scrollWidth = clientWidth = 390`，暗色背景 `rgb(7, 12, 20)`，无白色大块；控制台 0 error、0 warning，仅 React DevTools info。

验收：`npm run test -- src/App.test.tsx --run -t "提交成功后停留"` 通过；`npm run typecheck` 通过；`npm run verify:quick` 通过 11 个测试文件 / 197 个测试。

### R348：成长档案加入通关收藏解锁仪式

- [x] 在章节结算与真实成长档案结案页加入 `收藏解锁仪式`：通关后不只显示 XP，还会显示本关可写入图鉴的角色/宠物/装备、对应能力印记，以及下一位会遇见的收藏目标。
- [x] 第 1 章真实结案路径现在显示 `伙伴 · 档案馆记录员`、能力印记 `前端、接口、数据库、持久化、证据链`，以及下一位 `宠物 · 灵感萤火`，把奖励从数字反馈变成可收集目标。
- [x] 文案保留学习边界：收藏卡只说明本关证据已提交，真正熟练还要靠后续复测和真实项目迁移，避免把 XP 或收藏物包装成能力证明。
- [x] 补充渲染回归测试：`ChapterRewardGate` 必须把 XP 转成收藏角色和下一位预告；`CareerDossier` 真实提交后也必须显示收藏解锁仪式。
- [x] 真实浏览器复核：隔离 API `4428`、临时 SQLite `/tmp/code-quest-r349.sqlite`、Vite `5278` 下，从 `#chapter-1` 准备已通过 attempt，点击 `生成成长档案` 后真实结案页显示 `收藏解锁仪式 / 伙伴 · 档案馆记录员 / 下一位会遇见 / 宠物 · 灵感萤火`。390×844 手机视口 `scrollWidth = clientWidth = 390`，暗色背景 `rgb(7, 12, 20)`，无白色大块；控制台只有 React DevTools 提示。

验收：`npm run test -- src/App.test.tsx --run -t "章节结算会把 XP|提交成功后停留"` 通过；`npm run typecheck` 通过；`npm run verify:quick` 通过 11 个测试文件 / 197 个测试。

### R347：Lab 实战接力文案区分剧情交接和上一题收录

- [x] 修复第 1 章进入 Lab 后继续到第二个实战步骤时的理解断点：`实战追踪` 不再把每一步都说成“剧情教学已经交接”，避免用户误以为每次换地点都从剧情重新开始。
- [x] 第一题 `读取项目材料` 仍保留剧情到实战的交接说明；第二题及以后改为显示 `上一题「读取项目材料」已经收录；现在从「还原数据流」继续……`，明确“上一棒已经保存，现在只是同一条证据链继续往下传”。
- [x] 补充长流程回归测试：用户保存 `读取项目材料` 后，`实战追踪` 必须显示上一题已收录和当前题继续，且不得出现 `所以实战从「还原数据流」开始` 这种误导文案。
- [x] 真实浏览器复核：隔离 API `4427`、临时 SQLite `/tmp/code-quest-r348.sqlite`、Vite `5277` 下，`#chapter-1` 停在 `还原数据流`，冒险日志显示上一题已收录、当前继续；390×844 手机视口 `scrollWidth = clientWidth = 390`，暗色背景 `rgb(7, 12, 20)`，无白色大块；控制台只有 React DevTools 提示。

验收：`npm run test -- src/App.test.tsx --run -t "进入主线后进入教学地图"` 通过；`npm run typecheck` 通过；`npm run verify:quick` 通过 11 个测试文件 / 196 个测试。

### R346：第 1 章进入 Lab 后刷新不再掉回旧教学地图

- [x] 修复第 1 章从伙伴会合进入实战 Lab 后 URL 变成根路径的问题；现在进入 Lab 会保留对应章节 hash，避免刷新后丢失章节上下文。
- [x] 深链恢复逻辑新增剧情调查完成判断：`canvasstorm-investigation` 等剧情进度已完成时，`#chapter-1` 会直接恢复到 Lab，不再误判为旧教学步骤未完成。
- [x] Lab 冒险日志新增交接说明：`剧情教学已经把「任务委托」整理成委托草案；所以实战从「读取项目材料」开始。你没有漏步骤……`，解释为什么进入实战后从第 2 步开始读材料。
- [x] 保留教学日志中的同类交接说明，帮助从教学桥进入下一步时理解“不是漏步骤，是上一棒已经交接”。

验收：`npm run test -- src/App.test.tsx --run -t "进入主线后进入教学地图|岗位章节深链恢复|第 1 章剧情调查完成"` 通过；`npm run verify:quick` 通过 11 个测试文件 / 196 个测试。真实浏览器复核：隔离 API `4426`、临时 SQLite `/tmp/code-quest-r347.sqlite`、Vite `5276` 下，`#chapter-1` 刷新后直接显示 `主线 1-1 · AI 应用开发 实战追踪`，不再出现 `数据断层项目地图`；冒险日志显示 `你没有漏步骤` 交接说明。390×844 下 `scrollWidth = clientWidth = 390`，暗色背景 `rgb(7, 12, 20)`，无白色大块；控制台只有 React DevTools 提示。

### R345：下一地点预告补证据交接，避免用户忘记为什么换场景

- [x] 修复第 1 章第三幕之后的场景衔接：`下一地点预告` 不再只说“去下一幕”，而是明确显示 `带着这份证据` 和 `下一幕要交出`。
- [x] 第 1 章从数据库反证进入修复台时，会让用户看到当前证据是 `SELECT 查询却是 0 行`，下一幕要交出的材料是 `保存后刷新，再查数据库`，避免用户忘记上一站为什么重要。
- [x] 新增暗色小证据卡样式，保持 RPG 界面一致，不新增白色卡片断层。
- [x] 补充渲染回归测试：第三地点收集 `照亮空档案格` 和 `比对读写路径` 后，`下一地点预告` 必须显示证据、下一幕交付和数据库反证。

验收：`npm run test -- src/App.test.tsx --run -t "进入主线后进入教学地图"` 通过；`npm run verify:quick` 通过 11 个测试文件 / 195 个测试。真实浏览器复核：隔离 API `4425`、临时 SQLite `/tmp/code-quest-r346b.sqlite`、Vite `5275` 下，从 `#chapter-1` 走完第一幕和第二幕的判断/主动复述，进入第三地点后收集 `照亮空档案格` 与 `比对读写路径`；`下一地点预告` 显示 `带着这份证据 / SELECT 查询却是 0 行 / 下一幕要交出 / 保存后刷新，再查数据库`。桌面 `1200px` 与手机 `390×844` 均 `scrollWidth = clientWidth`，暗色背景 `rgb(7, 12, 20)`，未发现白色大块；控制台只有 React DevTools 提示。

### R344：场景复盘跟随最后关键线索，避免复盘抓错证据

- [x] 修复教学剧情 `本幕复盘三段式` 和 `面试一句话` 固定使用第一条线索的问题；现在复盘会优先使用当前查看的线索，兜底使用本幕最后一条线索。
- [x] 第 1 章第一幕收尾时，复盘证据会落在 `追踪发出的委托`，说明“舞台把要保存的画布数据交给传送门”，不再只停留在 `检查绿色灯牌`。
- [x] 第 1 章第二幕收尾时，复盘证据会落在 `审问守卫的证词`，明确“真正的落库证据必须来自数据库查询”，面试一句话也同步使用这条关键证据。
- [x] 补充渲染回归测试：第一幕复盘必须显示 `追踪发出的委托`，第二幕复盘必须显示 `审问守卫的证词` 和 `真正的落库证据必须来自数据库查询`。
- [x] 真实浏览器复核：隔离 API `4423`、临时 SQLite `/tmp/code-quest-r345.sqlite`、Vite `5273` 下，从 `#chapter-1` 走到第二地点并收集 2/2 线索；桌面与 390×844 手机均显示第二幕复盘证据为 `审问守卫的证词`，`scrollWidth = clientWidth`，暗色背景 `rgb(7, 12, 20)`，未发现白色块。

验收：`npm run test -- src/App.test.tsx --run -t "进入主线后进入教学地图"` 通过；`npm run verify:quick` 通过 11 个测试文件 / 195 个测试。后续继续推进第三地点和实战 Lab 的整章浏览器录入式验收。

### R343：主动复述改成证据专属，不再泛泛说“下一层”

- [x] 修复第 1 章第一幕主动复述提示过于抽象的问题：原文 `它还不能证明：下一层已经真的完成` 改成根据当前流程自动生成具体证据边界。
- [x] 第一章第一幕收完线索后，提示会明确写出：`Network 里看到 201 Created` 只能证明后端回信成功，还不能证明 `数据库真的完成`，因为还没看到 `SELECT 查询却是 0 行`。
- [x] 下一幕提示也改成具体交接：`下一幕我要查：传送门大厅的证据，也就是后端接口 → 数据层函数`，避免用户进入第二地点后忘记为什么要继续。
- [x] 补充渲染回归测试：锁定主动复述卡必须显示 `Network 里看到 201 Created`、`数据库真的完成`、`SELECT 查询却是 0 行`、`后端接口 → 数据层函数`。
- [x] 真实浏览器复核：隔离 API `4422`、临时 SQLite `/tmp/code-quest-r344.sqlite`、Vite `5272` 下，从 `#chapter-1` 点击开始、收集 2/2 线索、选择继续追证据、封存复述并进入第二地点；页面显示上一幕回声、判断回声和第二地点当前交接单。390×844 手机视口 `scrollWidth = clientWidth = 390`，暗色背景 `rgb(7, 12, 20)`，未发现白色块。

验收：`npm run test -- src/App.test.tsx --run -t "进入主线后进入教学地图"` 通过；`npm run verify:quick` 通过 11 个测试文件 / 195 个测试。后续仍需继续从第二地点收集线索走到第三地点/实战 Lab，检查整章是否都能保持这种证据边界。

### R342：第 1 章当前流程加入四格交接单

- [x] 修复第 1 章教学剧情里“谁把什么交给谁”仍偏抽象的问题：`当前流程定位` 下新增 `当前流程交接单`，把当前这一棒拆成 `收到什么 / 谁来处理 / 交出什么证据 / 下一步看哪里`。
- [x] 交接单默认显示在完整流程卷轴上方，用户不用展开完整地图也能知道当前地点的输入、处理者、可观察证据和下一棒方向；完整流程仍保持折叠，避免手机首屏信息过载。
- [x] 手机端样式改为两列暗色证据卡，保持 RPG 暗色风格，不新增白色卡片或后台断层。
- [x] 补充渲染回归测试：第 1 章第一幕默认显示 `点击保存，把画布名字交给页面`、`你能看到按钮变成“保存成功”`；点击第一条线索后，交接单切到 `POST /api/canvases 请求`、`Network 里看到 201 Created`、`后端接口 → 数据层函数`。
- [x] 真实浏览器复核：隔离 API `4421`、临时 SQLite `/tmp/code-quest-r343.sqlite`、Vite `5271` 下，`#chapter-1` 点击「开始闯关」后，桌面 `1280px` 与手机 `390×844` 均 `scrollWidth = clientWidth`，暗色背景 `rgb(7, 12, 20)`，未发现白色块；手机下交接单为两列 `162px 162px`。

验收：`npm run test -- src/App.test.tsx --run -t "当前流程|进入主线"` 通过；`npm run test -- src/App.test.tsx --run -t "第 3 章从路线卷宗|第 4 章从路线卷宗|第 5 章从路线卷宗|第 6 章从路线卷宗|第 7 章从路线卷宗"` 通过；`npm run verify:quick` 通过 11 个测试文件 / 195 个测试。后续仍需从第二地点继续做完整流程到实战 Lab 的浏览器录入式复核。

### R341：第 1 章主动复述加入三句式小抄

- [x] 修复第 1 章第一幕收集完线索后的学习摩擦：`本幕主动复述` 不再只提示“至少写 12 个字”，新增 `照着这三句写` 小抄。
- [x] 小抄固定引导用户回答三件事：这条证据证明什么、还不能证明什么、下一幕要查什么；帮助新手把“看懂页面”变成可复述的工程判断。
- [x] 补充渲染回归测试：第 1 章第一幕选择正确判断后，主动复述卡显示三句式提示，并明确下一幕要查 `传送门大厅` 的证据。
- [x] 真实浏览器复核：隔离 API `4420`、临时 SQLite `/tmp/code-quest-r342.sqlite`、Vite `5270` 下，第 1 章第一幕收集 2/2 线索并选择“沿证据继续追到后端接口”后，页面显示 `照着这三句写`；390×844 手机视口 `scrollWidth = clientWidth = 390`，暗色背景 `rgb(7, 12, 20)`，未发现白色块。

验收：`npm run test -- src/App.test.tsx --run -t "进入主线后进入教学地图"` 通过。后续仍需继续从主动复述进入第二地点，再一路走到伙伴会合和实战 Lab。

### R340：第 1 章地点航线默认收起，手机首屏先看当前地点

- [x] 修复第 1 章教学剧情的首屏节奏：`本章地点航线` 不再默认铺开全部 4 个地点，改为固定显示 `当前地点定位`，让新手先知道“现在只看这一站”。
- [x] 完整地点路线保留为 `details.quest-scene-rail-details`，需要复盘地图时再展开；这与 R339 的完整流程卷轴收起保持同一套阅读节奏。
- [x] 补充渲染回归测试：第 1 章进入教学剧情后，`当前地点定位` 可见，`完整地点路线` 默认没有 `open` 属性。
- [x] 真实浏览器复核：隔离 API `4419`、临时 SQLite `/tmp/code-quest-r341.sqlite`、Vite `5269` 下，`#chapter-1` 点击「开始闯关」后，390×844 手机视口 `scrollWidth = clientWidth = 390`，暗色背景 `rgb(7, 12, 20)`，未发现白色块，`placeRouteOpen = false` 且 `flowOpen = false`。

验收：`npm run test -- src/App.test.tsx --run -t "完整地点路线|完整流程需要用户主动展开"` 通过 2 项目标测试。后续仍需继续做第 1 章从线索收集到实战 Lab 的完整浏览器复核。

### R339：教学剧情流程卷轴默认收起，降低手机首屏信息过载

- [x] 修复教学剧情页的流程展示节奏：`完整流程卷轴` 不再默认展开完整 5 棒路线，改为先固定显示 `当前流程定位`，让新手先知道“现在谁把什么交给谁”。
- [x] 完整流程仍保留在可展开的 `details.quest-flow-board` 中，需要全局复盘时用户再展开，不影响后续理解完整链路。
- [x] 补充渲染回归测试：前端第 5 关进入教学剧情后，`当前流程定位` 可见，`完整流程` 默认没有 `open` 属性。
- [x] 真实浏览器复核：隔离 API `4418`、临时 SQLite `/tmp/code-quest-r340.sqlite`、Vite `5268` 下，`#chapter-frontend-5` 点击「开始闯关」后，390×844 手机视口 `scrollWidth = clientWidth = 390`，暗色背景 `rgb(7, 12, 20)`，未发现白色块，`fullFlowOpen = false`。

验收：`npm run test -- src/App.test.tsx --run -t "完整流程需要用户主动展开|进入主线后进入教学地图"` 通过 2 项目标测试。后续仍需继续做全站关键路径视觉终审和真人试玩。
...
- [x] 修复 `LabFlowDialogue` 首尾 fallback：第一棒上一棒显示 `剧情现场 / 事故线索`，最后一棒下一棒显示 `结案卷宗 / 最终验收`，避免流程自己交给自己。
- [x] 增加渲染回归测试：岗位 Lab 第一棒必须显示 `剧情现场`，且不得出现 `${currentFlow.label} 把线索交给 ${currentFlow.label}`。

验收：`npm run test -- src/App.test.tsx --run -t "岗位 Lab 默认|前端第 5"` 通过 2 项目标测试；`npm run verify:quick` 通过 lint、typecheck、11 个测试文件 / 189 个测试。浏览器隔离 API `4408`、临时 SQLite `/tmp/code-quest-r332.sqlite`、Vite `5258` 下，前端第 5 关深链桌面 `1280` 与手机 `390×844` 均无横向溢出，暗色背景 `rgb(7, 12, 20)`，控制台 error 为 0，正文不出现 `/api/canvases`、`canvas-save-persistence`、保存画布、验收试炼画布、`response.ok 后显示 saved`、数据库真的写入、保存后刷新或保存链路。
