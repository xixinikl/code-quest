...
- [x] `HANDOFF.md` 顶部事实快照更新到 `d77761a feat(rpg): anchor code tour lines to evidence`，避免新 Agent 或另一台电脑误把旧提交当最新状态。
- [x] `docs/cx-ai-career-rpg-home-merge-notes.md` 更新最新提交、远端 hash、约 65% 阶段完成度、最新快速验证和合并前必须重新完整验证的边界。
- [x] 当前本地分支 `cx/ai-career-rpg-home` 已核对等于 `origin/cx/ai-career-rpg-home`，远端 hash 为 `d77761a8110789af61d72883cbd2191948b32fb9`。
