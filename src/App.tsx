  ...
  [CASE11_SCENARIO_ID]: {
    ...
    steps: [
      ...
      },
      {
        ...
        kind: "response",
        response: {
          title: "阶段 04 · 筛选请求后再核对可见列表",
          prompt: "用 Network 复测说明集成测试和手动路径分别能证明什么",
          placeholder:
            "GET /api/tasks?status=blocked 返回……DOM 可见行显示……POST /api/reports/verification 返回……这证明本次前端路径……但不能单独证明……还需要……",
          minimum: 100,
          instruction:
            "集成测试关注模块交接。这里要把筛选请求、DOM 可见结果和报告提交串起来，而不是只说“接口没报错”。",
...
labConfigs[JAVA_CACHE_SCENARIO_ID] = {
  ...
  flowItems: [
    {
      label: "用户",
      title: "读到旧状态",
      detail: "先把体感异常翻译成一次可复现读取",
    },
    {
      label: "Network",
      title: "确认请求时间线",
      detail: "看第一次和第二次请求是否命中缓存",
    },
    {
      label: "Service",
      title: "拆缓存逻辑",
      detail: "区分 cache hit、DB version 和异步刷新",
    },
    {
      label: "日志",
      title: "串起版本证据",
      detail: "用 requestId、version 和 TTL 判断谁交出了旧值",
    },
    {
      label: "复测",
      title: "证明会收敛",
      detail: "更新后再次读取，确认缓存不会继续吐旧数据",
    },
  ],
  baseline: {
    label: "Java 缓存观测委托已领取",
    title: "先别只说缓存有问题，画出旧数据从哪来",
    body: "用户刚改完项目状态却读到旧版本。你要沿 Network、Java Service、缓存命中、数据库版本和后台日志判断：旧值到底是缓存吐出来的、数据库没更新，还是异步刷新还没完成。",
    action: "进入缓存风廊",
  },
  practical: {
  ...
  result: {
    ...
    nextItems: [
      "看到旧数据时，先问这次读取来自缓存、数据库还是异步任务。",
      "用 requestId、version、TTL 和 cache hit/miss 串起运行时证据。",
      "面试中说明缓存不是加速口号，而是必须能观测、能失效、能复测收敛。",
    ],
  },
  hints: [
    "先看 Network：同一个项目状态更新后，第二次读取是否仍返回旧 version。",
    "再看 ProjectCacheService：它从缓存、数据库和异步刷新里分别拿了什么值。",
    "最后看 backend.log 和 cache-retest：用 requestId、cache hit/miss、version 和 TTL 证明旧数据会不会收敛。",
  ],
...
labConfigs[JAVA_INCIDENT_SCENARIO_ID] = {
  ...
  flowItems: [
    {
      label: "报警",
      title: "确认影响窗口",
      detail: "错误率、P95 和业务成功率先说明事故是否真实",
    },
    {
      label: "日志",
      title: "串 requestId",
      detail: "用时间、版本、异常栈和请求 ID 对齐现场",
    },
    {
      label: "范围",
      title: "判断影响用户",
      detail: "区分单接口、单租户、全量用户和数据风险",
    },
    {
      label: "止血",
      title: "选择降级或回滚",
      detail: "先恢复核心路径，再决定是否保留新版本",
    },
    {
      label: "复测",
      title: "证明恢复",
      detail: "回滚后用冒烟、指标和日志证明真的恢复",
    },
  ],
  baseline: {
    label: "事故回声委托已领取",
    title: "先别只说线上挂了，把报警和日志串起来",
    body: "这关训练线上故障排查。你要从错误率、P95 延迟、requestId、异常堆栈、版本号、影响用户、止血动作和回滚复测里判断：故障到底影响了谁，应该降级还是回滚，恢复证据是否可信。",
    action: "进入事故回声塔",
  },
  practical: {
  ...
  result: {
    ...
    nextItems: [
      "事故第一步不是猜根因，而是确认影响窗口、受影响用户和核心指标。",
      "用 requestId、版本号、异常栈和时间线把日志串成可复核证据。",
      "面试中把止血、回滚、恢复验证和复盘取舍讲清楚，而不是只说修好了。",
    ],
  },
  hints: [
    "先看 monitoring-snapshot：错误率、P95 和业务成功率是否已经越过事故阈值。",
    "再看 IncidentTimeline.java 和 backend.log：requestId、异常栈、版本号和时间窗口能不能对上。",
    "最后看 rollback-plan 和 smoke-test：回滚条件、稳定版本、回滚后冒烟和指标恢复是否闭环。",
  ],
  steps: labConfigs[CASE14_SCENARIO_ID].steps.map((step) =>
    rewriteLabCopy(step, [
      ["上线", "事故处理"],
...
labConfigs[FRONTEND_ACCESSIBILITY_SCENARIO_ID] = {
  ...
  flowItems: [
    {
      label: "用户任务",
      title: "确认关键动作",
      detail: "先说清按钮、表单和错误提示要完成什么",
    },
    {
      label: "语义结构",
      title: "按钮和区域可识别",
      detail: "不用 div 冒充按钮，状态要能被辅助技术理解",
    },
    {
      label: "键盘路径",
      title: "不用鼠标也能完成",
      detail: "Tab 顺序、焦点样式和 Enter/Space 都要可用",
    },
    {
      label: "状态反馈",
      title: "错误和成功可感知",
      detail: "aria-live、错误文案和视觉对比一起交代结果",
    },
    {
      label: "390px 复测",
      title: "移动端不溢出",
      detail: "小屏、读屏和旧功能回归都要留下证据",
    },
  ],
  baseline: {
    label: "无障碍交付委托已领取",
    title: "先证明漂亮页面真的可用",
    body: "这关不问页面好不好看，而是问键盘用户、读屏用户和 390px 手机用户能不能完成同一件事。你要用语义、焦点、错误反馈、对比度、移动端和回归测试证明交付完整。",
    action: "进入无障碍交付庭",
  },
  practical: {
  ...
  result: {
    ...
    nextItems: [
      "不要把截图好看当成交付完成，要证明不同用户都能完成关键任务。",
      "用语义、焦点、错误反馈、对比度和移动端复测组成验收证据。",
      "面试中把无障碍讲成真实用户路径和交付质量，而不是背规范名词。",
    ],
  },
  hints: [
    "先看组件是不是用语义按钮和可识别区域，而不是只靠点击事件和视觉样式。",
    "再用键盘路径检查 Tab 顺序、焦点样式、Enter/Space 和错误提示是否可感知。",
    "最后用 390px、自动检查和回归说明证明它不是只在桌面鼠标路径好看。",
  ],
  steps: labConfigs[CASE14_SCENARIO_ID].steps.map((step) =>
    rewriteLabCopy(step, [
      ["上线", "前端交付"],
...
const glossaryExplanations: Record<string, string> = {
  ...
  数据库: "长期保存数据的地方，刷新页面后还应该能从这里读回来。",
  持久化: "把内存里的临时数据写进可长期保存的位置。",
  "201": "创建成功的 HTTP 状态码，说明接口说“我已经新建了一条记录”。",
  SELECT: "数据库查询语句，用来证明记录到底有没有写进去。",
  "Project Brief": "项目背景卡：说明用户、目标、输入、输出和当前不做什么。",
  候选方案: "AI 或系统给出的多个可选方向，还没决定最终采用哪一个。",
  会话: "一次连续使用过程留下的上下文和结果记录。",
  状态: "页面或系统当前记住的信息，比如是否保存、是否登录。",
  取舍: "说明为什么选一个方案、暂时放弃另一个方案。",
  Cookie: "浏览器自动带给同一网站的小门牌，常用来保存登录凭证。",
  Session: "后端保存的登录登记册，用来认出这个用户是谁。",
  Token: "一串代表身份的凭证，请求时带给后端验证。",
  "401": "未登录或凭证无效的状态码，后端在说“我认不出你”。",
  过期时间: "凭证什么时候失效，过了就需要重新登录或刷新凭证。",
  请求参数: "前端交给接口的数据，包括路径、查询、请求体和请求头。",
  状态码: "接口返回的数字结果，告诉你成功、参数错、未登录还是服务器异常。",
  "400": "请求参数有问题，通常是前端传少了或传错了字段。",
  "500": "服务器内部异常，通常要查后端日志和异常栈。",
  日志: "后端留下的运行记录，用来追踪某次请求发生了什么。",
  错误边界: "把错误拦住并展示可理解提示的保护层。",
  唯一键: "保证一类数据不能重复的标记，比如同一个订单号只能出现一次。",
  幂等: "同一个动作重复执行多次，结果仍然像执行一次。",
  并发: "多个请求几乎同时发生，可能互相抢数据。",
  事务: "一组数据库操作要么一起成功，要么一起失败。",
  重复提交: "用户或网络让同一个请求发了多次，可能写出多份数据。",
  加载: "资源、数据或页面开始准备并显示出来的过程。",
  渲染: "浏览器把数据和组件画成用户能看到的界面。",
  TTFB: "从发出请求到收到第一个响应字节的时间，常用来判断后端慢不慢。",
  缓存: "把结果临时存起来，下次少算或少请求。",
  瀑布图: "Network 里展示每个请求耗时顺序的图。",
  "API Key": "调用外部服务的钥匙，必须放在服务端或环境变量里保护。",
  环境变量: "不同环境使用的配置，比如密钥、接口地址和开关。",
  流式响应: "结果一段段返回，用户不用等全部生成完。",
  限流: "限制请求频率，避免服务被打爆或费用失控。",
  超时: "请求太久没结果时主动放弃并给出兜底。",
  Prompt: "给 AI 的任务说明，决定它按什么方向回答。",
  上下文: "AI 回答时能看到的背景材料和历史信息。",
  引用: "回答中标明信息来自哪里，方便复查。",
  幻觉: "AI 看起来很肯定，但内容没有证据支撑。",
  置信边界: "明确哪些答案可靠，哪些需要继续查证。",
  RAG: "先检索资料，再把资料交给 AI 回答的流程。",
  chunk: "把长文档切成的小段，方便检索。",
  embedding: "把文字变成向量，方便机器比较相似度。",
  检索: "从知识库里找出最相关的材料。",
  命中率: "检索结果有没有找对资料的比例或表现。",
  工具调用: "让 Agent 调用某个受控能力，比如查数据库或读文件。",
  "参数 schema": "规定工具参数长什么样，防止 Agent 乱传。",
  权限: "规定谁能做什么，哪些操作不能越界。",
  回退: "失败时恢复到安全状态或换备用方案。",
  审计: "留下操作记录，之后能追踪谁做了什么。",
  单元测试: "验证一个小函数或模块是否正确。",
  集成测试: "验证多个模块连起来是否工作。",
  手动测试: "人按真实路径操作并记录结果。",
  回归: "确认修复没有把旧功能弄坏。",
  证据: "能被复查的材料，比如 Network、日志、数据库和测试报告。",
  任务背景: "告诉 Agent 为什么要做这件事，以及当前问题是什么。",
  约束: "明确不能做什么、不能碰哪里、必须遵守什么边界。",
  验收标准: "怎样证明任务已经完成，而不是看起来完成。",
  风险: "可能出错、影响用户或影响上线的地方。",
  diff: "代码改动对比，用来审查到底改了什么。",
  回归风险: "新改动可能把旧路径弄坏的风险。",
  边界条件: "不常见但必须处理的输入或状态。",
  迁移: "把数据、配置或逻辑从旧方式转到新方式。",
  交付说明: "Agent 或开发者说明改了什么、如何验收、还有什么风险。",
  构建: "把代码打包成可运行产物。",
  备份: "上线或改数据前保存一份可恢复副本。",
  冒烟测试: "上线前快速走核心路径，确认没有明显故障。",
  回滚: "出问题后退回上一个稳定版本。",
  STAR: "面试复盘结构：情境、任务、行动、结果。",
  复盘: "把发生了什么、怎么判断、怎么改进讲清楚。",
  技术取舍: "说明为什么选这个方案，而不是另一个方案。",
  成长证据: "能证明你能力提升的项目记录、测试和复盘。",
  追问: "面试官继续深挖细节的问题。",
};

function LabGlossaryStrip({ config }: { config: LabConfig }) {
  const { chapter } = getRouteChapterForScenarioId(config.scenarioId);
  if (!chapter?.glossary.length) return null;

  return (
    <section className="lab-glossary-strip" aria-label="本关名词小抄">
      <header>
        <span>名词小抄</span>
        <strong>先用人话记住这些词，再去看证据</strong>
      </header>
      <div>
        {chapter.glossary.slice(0, 6).map((term) => (
          <article key={term}>
            <BookOpen aria-hidden="true" />
            <strong>{term}</strong>
            <p>
              {glossaryExplanations[term] ??
                "本关会反复出现的关键词，先放回流程里看它收到什么、交出什么。"}
            </p>
          </article>
        ))}
      </div>
